...
def extract_and_copy_data(input_file, output_file):
    ...
    for record in data:
        extracted_record = {
            "nop": record['data_pembayaran']['nop'],  # Fixed incorrect use of .get
            "tahun_pajak": record['data_pembayaran']['tahun']  # Fixed incorrect use of .get
        }
...
def add_bank_and_merchant_codes(output_file, config_data):
    ...
    with open(output_file, 'w') as outfile:
        json.dump(data, outfile, indent=4, ensure_ascii=False)  # Added ensure_ascii=False for better character encoding
...
update_pbb_determination(f'SW_PBB/pbb_data_payment/pbb_data_pembayaran.json', output_file)