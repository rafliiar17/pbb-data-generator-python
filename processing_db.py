...
def export_json_to_mongo(db, json_file_path, collection_name):
    try:
        with open(json_file_path, 'r') as file:
            ...
            total = len(data)
            for i, document in enumerate(tqdm(data)):  # Utilize tqdm for progress indication
...
export_json_to_mongo(db1, json_file_path_assesment, collection_name_assesment)
